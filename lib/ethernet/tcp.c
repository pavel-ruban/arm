...
void tcp_data(uint8_t id, eth_frame_t *frame, uint16_t len, uint8_t closing)
{
	...
	if ((!closing) && (len))
	{
		memcpy(tcp->data,
				...
				"\r\n"
				"<html><head><title>SOMI internal MCU settings form</title></head><body><form><input type=\"text\" name=\"mac_address\" /><input type=\"submit\" value=\"ok\"/></form></body></html>\r\n"
				,
				224);
		tcp_send(id, frame, 224, 1, 0);
		return;
	}
	...
	if(!st)
	{
		...
		if( tcpflags == TCP_FLAG_SYN )
		{
			...
			while (!st)
			{
				...
				if (i) break;
				// If client asks for connection but currently all
				// connections used check if there are some expired and check slot again.
				tcp_poll();
				++i;
			}
lookup_end:
...
void tcp_poll()
{
	...
	for(id = 0; id < TCP_MAX_CONNECTIONS; ++id)
	{
		...
		if( (st->status != TCP_CLOSED) &&
				(RTC_GetCounter() - st->rx_time > TCP_TIMEOUT) )
		{
			// Прибиваем его
			st->status = TCP_CLOSED;
